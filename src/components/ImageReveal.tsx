...
export function ImageRevealSection() {
  ...
  // Mouse tilt variables (desktop only)
  ...
  // Circular progress calculations (desktop only)
  ...
  return (
    <>
      {/* MOBILE & TABLET LAYOUT: Responsive Grid Column Layout */}
      <div className="block md:hidden bg-black text-white py-16 px-4 sm:px-6 w-full">
        <div className="max-w-7xl mx-auto">
          {/* Section Header */}
          <div className="mb-12 text-center">
            <span className="text-xs uppercase tracking-widest text-[#8A78B4] font-bold block mb-2">// SELECTED WORK</span>
            <h2 className="text-3xl sm:text-5xl font-bold tracking-tight text-white font-display">Featured Projects</h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full">
            {projectsData.map((project) => (
              <div 
                key={project.id} 
                className="flex flex-col bg-[#111111] rounded-2xl overflow-hidden border border-white/5 transition-all duration-500 hover:border-[#8A78B4]/30 hover:shadow-[0_10px_30px_rgba(138,120,180,0.15)]"
              >
                {/* Visual Area */}
                <div className="w-full aspect-[4/3] overflow-hidden relative">
                  <img 
                    ...
                    loading="lazy"
                  />
                  <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md border border-white/10 rounded-full px-3 py-1 text-[10px] uppercase font-bold tracking-widest text-[#8A78B4]">
                    {project.number}
                  </div>
                </div>
                ...
                <div className="p-6 flex-1 flex flex-col justify-between gap-4">
                  <div>
                    <h3 className="text-lg sm:text-xl font-bold text-white font-display mb-2">{project.title}</h3>
                    <p className="text-xs sm:text-sm text-white/60 font-light leading-relaxed font-sans">{project.description}</p>
                  </div>

                  <button 
                    onClick={() => window.open(`https://wa.me/917510836988?text=${encodeURIComponent(`Hello Jishnu, I am interested in discussing: ${project.title}`)}`, "_blank")}
                    className="w-full h-12 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 text-white font-bold text-xs uppercase tracking-widest transition-all duration-300 hover:bg-[#8A78B4] hover:border-[#8A78B4]"
                  >
                    Inquire Project
                  </button>
                </div>
      ...
      {/* DESKTOP LAYOUT: Perfect Screen Stack Slider (Unchanged) */}
      <div 
        ref={containerRef} 
        className="hidden md:block relative w-full bg-black text-white" 
        style={{ height: "400vh" }}
      >
        ...
        <div className="sticky top-0 left-0 w-full h-screen overflow-hidden flex flex-col md:flex-row bg-black">
          ...
          <div className="w-full md:w-1/2 h-[20vh] md:h-full flex flex-col justify-between p-4 md:p-16 lg:p-24 z-10 select-none text-white!">
          ...
          <div 
            ...
            className="w-full md:w-1/2 h-[80vh] md:h-full relative overflow-hidden bg-black cursor-crosshair"
          >
